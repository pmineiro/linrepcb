...
# see tune-suppretrain.res
# 0.088697        doit(cuda=True, rank=800, seed=4545, initlr=0.030968434672789563, tzero=25.84162981589504, banditinitlr=0.0018601456569356024, bandittzero=59.69592899228503, epsilon=0.08254947494129913, epsilontzero=2.3539299786138406, prebs=32, banditbs=256) 1563    0.0003897       0.00037073      0.070417        0.088697        174.52
# see tune-spannerepsgreedydynamic.res
# 0.10726 train(mydata, model=model, actions=embeds, initlr=0.0017564107250362172, tzero=99.16424450426548, epsilon=0.08011268085183006, epsilontzero=1.5486085414492836, batch_size=256, pretrain=50000, cuda=True, seed=4545)        6516    0.28621 0.28999 0.10802 0.11372 0.10726 0.11312 6774.11859
#
# reuse hypers from k=1 but divide epsilon by 3 since r=3

def flass(seed):
    pretrain, depth = 50000, 2
    k, r = 5, 3
    rank, prebs, banditbs, cuda = 800, 32, 256, True
    initlr, tzero = 0.030968434672789563, 25.84162981589504
    banditinitlr, bandittzero = 0.0017564107250362172, 99.16424450426548
    epsilon, epsilontzero = 0.08011268085183006, 1.5486085414492836

    print(f'embeds = embed(mydata, batch_size={prebs}, pretrain={pretrain}, rank={rank}, seed={2*seed})')
    embeds = embed(mydata, batch_size=prebs, pretrain=50000, rank=rank, seed=2*seed)
    print(f'model = presup(mydata, actions=embeds, depth={depth}, initlr={initlr}, tzero={tzero}, batch_size={prebs}, pretrain={pretrain}, cuda={cuda}, seed={2*seed+1})')
    model = presup(mydata, actions=embeds, depth=depth,
                   initlr=initlr, tzero=tzero,
                   batch_size=prebs, pretrain=pretrain,
                   cuda=cuda, seed=2*seed+1)

    print(f'train(mydata, k={k}, r={r}, model=model, actions=embeds, initlr={banditinitlr}, tzero={bandittzero}, epsilon={epsilon/r}, epsilontzero={epsilontzero}, batch_size={banditbs}, pretrain={pretrain}, cuda={cuda}, seed={2*seed+1})')
    train(mydata, k=k, r=r, model=model, actions=embeds, 
          ...
          batch_size=banditbs, pretrain=pretrain, cuda=cuda, seed=2*seed+1)

import sys
start = int(sys.argv[1])
for myseed in range(start, start+100):
    flass(start)
